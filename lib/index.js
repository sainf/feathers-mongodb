...
class Service extends AdapterService {
  ...
  _aggregationPipeline(params) {
    const { filters, query } = this._options(params);


    if (typeof params.aggregate !== 'object' || !params.aggregate.length) {
      throw new errors.NotAcceptable('Check you aggregation array');
    }
  ...
  _find(params = {}) {
    ...
    const aggregationPipeline = params.aggregate !== undefined
      ? this._aggregationPipeline(params)
      : null

    const q = aggregationPipeline
      ? this.Model.aggregate(aggregationPipeline, options)
    ...
    // TODO Make count
    if (paginate && paginate.default && !aggregationPipeline) {
