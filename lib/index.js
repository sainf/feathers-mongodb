...
class Service extends AdapterService {
  ...
  _aggregationPipeline (query, options) {
    if (!options.$lookups) 
      return null

    // multiple lookups
    const $lookups = []
    // Bonus, convert array first result in to root object
    const $sets = {}

    // Use with custom lookup.ts hook
    if (options.$lookups) {
      options.$lookups.filter(l => {
        if (l.$singleDoc)
          $sets[l.as] = { $first: `$${l.as}` }
    ...
    if (Object.keys($sets).length) {
      aggregationPipeline.push({ $set: { ...$sets } })
    }
  ...
  _find (params = {}) {
    ...
    const aggregationPipeline = this._aggregationPipeline(query, options)
